...
def DM0Solver( Theo_mh,Exp_mh,seq,Error,dic_DM0):
...
def main(solverconfig,infile):
    ...
    for index, row in df.iterrows():
        if row[Seq_column_name].find("_") != -1: # If  scan has already been corrected DM0Seqeuence remains Seq_column_name
            ...
            DM0Sequence,DM0Label,DM0Lable_ppm = DM0Solver(row[Theo_mh_column_name],row[Exp_mh_column_name],row[Seq_column_name],Error,dic_DM0)
        ...
        df.loc[cont,DM0Label_ppm_output_column_name] = DM0Lable_ppm
        cont=cont+1
