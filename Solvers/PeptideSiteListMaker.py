...
def peptideSiteListMaker(df,seq,counts):
    ...
    """
    DM = seq[seq.find("[")+1:seq.find("]")] # DM of the sequence
    ...
    return clean_seq,seq,DM,aa,freq
...
def main(file,infile1):
    ...
    df = readInfile(infile1) # A dataframe is created based on input file
    ...
    for index, row in df.iterrows():
        ...
        if row[seq_column_name].find("_") == -1 and row[seq_column_name] not in seqlist:
            ...
            clean_seq,seq,DM,aa,freq=peptideSiteListMaker(df,row[seq_column_name],counts)
            

            df2.loc[cont+1,"EqSequence"] = clean_seq
            df2.loc[cont+1,"Species"] = seq 
            df2.loc[cont+1,"DM"] = DM
