...
def main(file,infile1):
    ...
    PeakAssignation_column_name =  config["PeptideSiteListMaker_Parameters"].get("PeakAssignation_column_name")
    PeakNaming =  config["PeptideSiteListMaker_Parameters"].get("PeakNaming")
    Outfile_suffix =  config["PeptideSiteListMaker_Parameters"].get("Outfile_suffix")
    ...
    for index, row in df.iterrows():
        ...
        if row[seq_column_name].find("_") == -1 and row[seq_column_name] not in seqlist and df.loc[cont,PeakAssignation_column_name].upper()==PeakNaming.upper():
