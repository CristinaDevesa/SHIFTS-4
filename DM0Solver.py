...
def DM0Solver( Theo_mh,Exp_mh,seq,Error,dicc_DM0):
    
    """
    DM0Solver function returns the sequence and label, that match with the smallest error
    according to the dicc_DM0 (dictionary with all possible options).
    Input:
         Theo_mh             > Theoretical mh  
         Exp_mh              > Experimental mh
         seq                 > Sequence with DM
         Error               > Error in ppms stablished by the user
         dicc_DM0            > Dictionary with the options of DM0List, in which label is the key and the corresponding mass the value.
    ...
    for Label in dicc_DM0:
        ...
        if DiffPPM <= Error:
            if DiffPPM < minimun_DiffPPM:
                ...
                DM0Label = Label
                DM0Lable_ppm = minimun_DiffPPM

   
    return DM0Sequence,DM0Label,DM0Lable_ppm
...
def main(solverconfig,infile):
    ...
    for index, row in df.iterrows():
        ...
        else: # if  scan has not yet been corrected DM0Sover function is executed
            DM0Sequence,DM0Label,DM0Lable_ppm = DM0Solver(row[Theo_mh_column_name],row[Exp_mh_column_name],row[Seq_column_name],Error,dicc_DM0)
   
            
        # New columns are completed  
        df.loc[cont,"DM0Sequence"] = DM0Sequence
        df.loc[cont,"DM0Label"] = DM0Label.upper()
        df.loc[cont,"DM0Label_ppm"] = DM0Lable_ppm
        cont=cont+1
